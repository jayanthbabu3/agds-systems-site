...
export function useEntrance<T extends HTMLElement>(delay = 0.15) {
  ...
  useEffect(() => {
    ...
    //
    // The 2s race is a hard failsafe, and it is not optional: without it any
    // context that reports hidden and never fires visibilitychange leaves the
    // page permanently blank with nothing in the console. Prerender, some
    // in-app browsers, and every headless/automation screenshot do exactly
    // that — which is also why hero screenshots came back empty.
...
export function useCountUp(target: number, duration = 1.9, locale = 'en-IN', decimals = 0) {
  ...
  useEffect(() => {
    ...
    return () => ctx.revert()
  }, [target, duration, locale, decimals])
